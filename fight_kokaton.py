...
class Bird:
    ...
    def update(self, key_lst: list[bool], screen: pg.Surface):
        ...
        if not (sum_mv[0] == 0 and sum_mv[1] == 0):
            self.dire = tuple(sum_mv)  # 移動方向をこうかとんの向きとして保存
...
class Beam:
    ...
    def __init__(self, bird:"Bird"):
        ...
        angle = math.degrees(math.atan2(-self.vy, self.vx)) # ビームの進行方向から回転角度を求める
        self.img = pg.transform.rotozoom(img, angle, 1.0) # ビーム画像を進行方向に合わせて回転

        self.rct = self.img.get_rect()
        self.rct = self.img.get_rect()
        self.rct.centerx = bird.rct.centerx + bird.rct.width * self.vx / 5 # 向きに応じてビームの横位置を調整
        self.rct.centery = bird.rct.centery + bird.rct.height * self.vy / 5 # 向きに応じてビームの縦位置を調整
    ...
    while True:
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:
                ...
                beams.append(Beam(bird))  # 新しいビームを生成してリストに追加          
        ...
        for i, beam in enumerate(beams):  # ビームと爆弾の衝突判定
            if beam is None:  # 削除済みのビームは処理しない
                continue
        ...
        for beam in beams: # すべてのビームを更新・描画
            beam.update(screen)
        for bomb in bombs:
            bomb.update(screen)
